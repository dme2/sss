...
public:
  ...
  void async_callback(snd_async_handler_t *handler) {
      ...
      AlsaBackend* data =
  (AlsaBackend*)snd_async_handler_get_callback_private(handler); auto avail = snd_pcm_avail_update(pcm_handle); float* buffer = new float[avail];
  ...
  */
  ...
  AlsaBackend *init_alsa_out(std::string out_device = "plughw:0,0") {
	...
	if (set_hw_params(this->handle, hw_params,  SND_PCM_ACCESS_RW_INTERLEAVED) < 0 )
	...
	unsigned int buffer_time = 500000;
	snd_pcm_hw_params_set_buffer_time_near(this->handle, hw_params, &buffer_time, nullptr);
